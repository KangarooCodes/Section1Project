...
// Mark story as favorite/non-favorite

function getHeartHTML(story, user) {
  const isFavorite = user.isFavorite(story);
  const heartType = isFavorite ? "solid" : "light";
  return `
      <span class="heart">
        <i class="${heartType} heart"></i>
      </span>`;
}
...
function putFavoritesListOnPage() {
  ...
  if (currentUser.favorites.length === 0) {
    $favoritedStories.append("<h5 id='no-favs'>No favorites added!</h5>");
  } else {
